...
namespace Headhunter
{
    ...
    {
        public override string ModuleName => "The Headhunter Acheivement";
        ...
        public Dictionary<CCSPlayerController, PlayerHeadShotKillCount> _playerKillCount { get; set; } = new();
        ...
        public void CountKill(CCSPlayerController client)
        {
            ...
            if (Config.MaxHeadshotKillCount <= 0)
                return;
            ...
            if (_playerKillCount[client!].KillCount >= Config.MaxHeadshotKillCount)
            {
                var steamid = client.AuthorizedSteamID?.SteamId64;
        ...
        public async Task TaskComplete(CCSPlayerController client, ulong steamid)
        {
            ...
            if (response != null)
            {
                ...
                Server.NextFrame(() =>
                {
                    client.PrintToChat($" {ChatColors.Green}[Acheivement]{ChatColors.Default} You acheive 'The Headhunter' (Kill enemies wih headshot for {Config.MaxHeadshotKillCount} times)");
